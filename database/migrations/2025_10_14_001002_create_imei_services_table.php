...
return new class extends Migration {
  public function up(): void {
    Schema::create('imei_services', function (Blueprint $t) {
      $t->increments('id');
      $t->string('icon',255)->nullable();
      $t->string('alias',255);                // INDEX لاحقًا
      ...
      $t->unsignedInteger('group_id')->nullable();       // FK -> service_groups
      $t->integer('source')->nullable();
      $t->unsignedInteger('remote_id')->nullable();
      $t->unsignedInteger('supplier_id')->nullable();    // FK -> suppliers
      $t->unsignedInteger('local_source_id')->nullable();// FK -> local_sources

      $t->boolean('device_based')->default(1);
      ...
      // فهارس/مفاتيح
      $t->index('alias','imei_services_alias_index');
      $t->index('group_id','imei_services_group_id_foreign');
      $t->index('remote_id','imei_services_remote_id_foreign');
      $t->index('supplier_id','imei_services_supplier_id_foreign');
      $t->index('local_source_id','imei_services_local_source_id_foreign');

      // مفاتيح خارجية (تعطيلها لو الجداول غير موجودة لديك)
  ...
  public function down(): void { Schema::dropIfExists('imei_services'); }
};
