...
class ServerOrdersController extends Controller
{
    private string $routePrefix = 'admin.orders.server';
    ...
    public function index(Request $r)
    {
        ...
        $services  = ServerService::query()->orderByDesc('id')->limit(500)->get();
        $providers = ApiProvider::query()->orderBy('name')->get();

        return view('admin.orders.server.index', compact('rows', 'services', 'providers'));
    }
    ...
    public function store(Request $r)
    {
        $data = $r->validate([
            ...
            'service_id' => ['required', 'integer', 'exists:server_services,id'],
            'device'     => ['required', 'string', 'max:255'],
            'comments'   => ['nullable', 'string'],
        ]);

        $user = User::findOrFail((int)$data['user_id']);
        $service = ServerService::findOrFail((int)$data['service_id']);
    ...
    public function modalView(ServerOrder $order)
    {
        $row = $order->load(['service', 'provider']);
        return view('admin.orders._modals.view', compact('row'));
    }

    public function modalEdit(ServerOrder $order)
    {
        $row = $order->load(['service', 'provider']);
        $routePrefix = $this->routePrefix;
    ...
    public function update(Request $r, ServerOrder $order)
    {
        $data = $r->validate([
