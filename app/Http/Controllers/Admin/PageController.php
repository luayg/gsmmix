...
final class PageController extends Controller
{
    ...
    public function update(Request $request, Page $page, HtmlSanitizer $sanitizer): RedirectResponse { $data=$this->validated($request,$page); DB::transaction(function()use($data,$page,$sanitizer){$page->update($this->pagePayload($data));$this->replaceTranslations($page,$data['translations'],$sanitizer);}); return back()->with('ok','Page updated.'); }
    ...
    private function pagePayload(array $data): array { return ['slug'=>$data['slug'],'status'=>$data['status'],'placement'=>$data['placement'],'parent_id'=>$data['parent_id']??null,'featured_image'=>$data['featured_image']??null,'og_image'=>$data['og_image']??null,'authenticated_only'=>(bool)($data['authenticated_only']??false),'open_new_window'=>(bool)($data['open_new_window']??false),'ordering'=>(int)($data['ordering']??0),'published_at'=>$data['status']==='published'?($data['published_at']??now()):($data['published_at']??null)]; }
