...
final class PageController extends Controller
{
    ...
    public function update(Request $request, Page $page, HtmlSanitizer $sanitizer): RedirectResponse { if($page->system){$request->merge(['slug'=>$page->slug,'status'=>'published','placement'=>$page->placement,'parent_id'=>$page->parent_id,'authenticated_only'=>(int)$page->authenticated_only,'ordering'=>$page->ordering,'published_at'=>optional($page->published_at)->toDateTimeString()]);} $data=$this->validated($request,$page); DB::transaction(function()use($data,$page,$sanitizer){$page->update($this->pagePayload($data,$page));$this->replaceTranslations($page,$data['translations'],$sanitizer);}); return back()->with('ok','Page updated.'); }
    ...
    private function pagePayload(array $data, ?Page $page=null): array { $system=$page?->system===true; return ['slug'=>$system?$page->slug:$data['slug'],'status'=>$system?'published':$data['status'],'placement'=>$system?$page->placement:$data['placement'],'parent_id'=>$system?$page->parent_id:($data['parent_id']??null),'featured_image'=>$data['featured_image']??null,'og_image'=>$data['og_image']??null,'authenticated_only'=>$system?$page->authenticated_only:(bool)($data['authenticated_only']??false),'open_new_window'=>(bool)($data['open_new_window']??false),'ordering'=>$system?$page->ordering:(int)($data['ordering']??0),'published_at'=>$system?($page->published_at??now()):($data['status']==='published'?($data['published_at']??now()):($data['published_at']??null))]; }
