...
{
    private const CONTENT_EDITABLE_SYSTEM_PAGES = ['privacy', 'terms'];
    ...
    public function index(Request $request)
    {
        ...
        $menuPages=Page::query()->with('translations')->where('status','published')->orderBy('ordering')->orderBy('id')->get();
        $themes=PageTheme::query()->orderByDesc('active')->orderByDesc('updated_at')->get();
        $originalThemeActive=!$themes->contains('active',true);
        return view('admin.pages.index',compact('pages','menus','menuPages','themes','originalThemeActive'));
    }
    ...
    {
        if ($page->system && !in_array($page->slug, self::CONTENT_EDITABLE_SYSTEM_PAGES, true)) {
            $data=$request->validate(['status'=>'required|in:draft,published','open_new_window'=>'nullable|boolean',...$this->styleRules()]);
            $page->update(['status'=>$data['status'],'open_new_window'=>$request->boolean('open_new_window'),'style'=>$this->stylePayload($data)]);
            return back()->with('ok','Core page appearance updated.');
        }
        if ($page->system) {
            $data=$request->validate([
                'status'=>'required|in:draft,published','authenticated_only'=>'nullable|boolean','open_new_window'=>'nullable|boolean',
                'translations'=>'required|array|min:1','translations.*.language_id'=>'required|distinct|exists:languages,id',
                'translations.*.title'=>'required|string|max:255','translations.*.content'=>'nullable|string|max:1000000',
                'translations.*.seo_title'=>'nullable|string|max:255','translations.*.seo_description'=>'nullable|string|max:1000',
                ...$this->styleRules(),
            ]);
            DB::transaction(function()use($request,$data,$page,$sanitizer){
                $page->update(['status'=>$data['status'],'authenticated_only'=>$request->boolean('authenticated_only'),'open_new_window'=>$request->boolean('open_new_window'),'style'=>$this->stylePayload($data)]);
                $this->replaceTranslations($page,$data['translations'],$sanitizer);
            });
            return back()->with('ok','Protected page content and access updated.');
        }
        $data=$this->validated($request,$page);
        DB::transaction(function()use($data,$page,$sanitizer){$page->update($this->pagePayload($data,$page));$this->replaceTranslations($page,$data['translations'],$sanitizer);});
        return back()->with('ok','Page updated.');
    }
    ...
    public function preview(Page $page){if($page->system){$route=match($page->slug){'home'=>'home','services'=>'site.services','store','products'=>'site.store','downloads'=>'site.downloads','place-order'=>'customer.orders.create','orders'=>'customer.orders','account','dashboard'=>'customer.dashboard','login'=>'login','register'=>'register',default=>null};if($route&&app('router')->has($route))return redirect()->to(route($route).'?page_preview='.$page->id);} return redirect()->to(route('site.page',$page).'?page_preview='.$page->id); }
    private function form(?Page $page=null){ return view('admin.pages.form',['page'=>$page,'contentEditableSystemPage'=>$page?->system && in_array($page->slug,self::CONTENT_EDITABLE_SYSTEM_PAGES,true),'languages'=>Language::query()->where('active',true)->orderBy('ordering')->get(),'parents'=>Page::query()->when($page,fn($q)=>$q->where('id','!=',$page->id))->orderBy('ordering')->get()]); }
