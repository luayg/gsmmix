...
final class AdminPermissions
{
    ...
    public static function required(?string $name, string $method, string $path, ?string $bulkAction = null): ?array
    {
        ...
        if (!$safe) {
            return in_array($action, ['update', 'reorder', 'import', 'toggle', 'activate', 'restore', 'sync', 'syncFields', 'set_overdraft', 'add_remove', 'add_payment', 'cancel', 'cache', 'approve', 'reject'], true)
                ? [$module . '.edit']
                : null;
        }
