...
test('admin dashboard exposes the live command center metrics',()=>{const view=readFileSync('resources/views/admin/dashboard.blade.php','utf8');for(const label of ['Today Orders','Today Registrations','Today Payments','Today Profit','Online Users','Order Acceptance Rate','Order Rejection Rate','Pending Approval','Payment Review Alerts','Recent Orders','Live Activity'])assert.match(view,new RegExp(label));assert.match(view,/data-refresh-seconds="60"/);assert.match(view,/window\.location\.reload\(\)/)});
