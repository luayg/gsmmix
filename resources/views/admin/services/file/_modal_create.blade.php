...
<form id="serviceCreateForm"
      ...
      action="{{ route('admin.services.file.store') }}"
      method="POST"
      data-ajax="1">
  @csrf
...
(function(){
  ...
  window.__fileServiceApplyRemoteFields__ = function(scope, additionalFields){
    try{
      ...
      if (droppedInvalid > 0) {
        console.warn(`file applyRemoteFields dropped ${droppedInvalid} invalid item(s)`);
      }
      if (droppedDuplicate > 0) {
        console.warn(`file applyRemoteFields dropped ${droppedDuplicate} duplicate item(s)`);
      }
      if (droppedByCap > 0) {
        console.warn(`file applyRemoteFields capped fields at ${MAX_REMOTE_FIELDS}`);
      }
