...
(function(){
  ...
  window.__imeiServiceApplyRemoteFields__ = function(scope, additionalFields){
    try{
      ...
      if (droppedInvalid > 0) {
        console.warn(`imei applyRemoteFields dropped ${droppedInvalid} invalid item(s)`);
      }
      if (droppedDuplicate > 0) {
        console.warn(`imei applyRemoteFields dropped ${droppedDuplicate} duplicate item(s)`);
      }
      if (droppedByCap > 0) {
        console.warn(`imei applyRemoteFields capped fields at ${MAX_REMOTE_FIELDS}`);
      }
      ...
      console.warn('imei applyRemoteFields failed', e);
    }
  };

  window.__imeiServiceSetMainField__ = function(scope, type, label){
    try{
      if (!scope) return;
