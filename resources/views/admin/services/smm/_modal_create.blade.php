{{-- resources/views/admin/services/smm/_modal_create.blade.php --}}

<form id="serviceCreateForm"
      class="service-create-form js-ajax-form"
      action="{{ route('admin.services.smm.store') }}"
      method="POST"
      data-ajax="1">
  @csrf
  ...
  <input type="hidden" name="main_type" id="mainTypeHidden" value="text">
        ...
        <div class="col-xl-7">
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <input name="time" type="text" class="form-control" placeholder="e.g. instant / 1-24h">
            </div>
            ...
            <div class="col-md-6">
              ...
              <select name="type" class="form-select">
                <option value="smm" selected>SMM</option>
              </select>
            </div>

            <div class="col-md-6">
              <label class="form-label mb-1">Main field label</label>
              <input name="main_field_label" id="mainFieldLabel" type="text" class="form-control" value="Target">
            </div>
            ...
            <div class="col-md-6">
              ...
              <div class="input-group">
                <input name="maximum" id="maxChars" type="number" class="form-control" value="500">
                <span class="input-group-text">Characters</span>
              </div>
            </div>
...
(function(){
  ...
  const presets = {
    text:   { label:'Target', allowed:'any',          min:1, max:500 },
    number: { label:'Quantity', allowed:'numbers',    min:1, max:20 },
  ...
  function buildMainFieldJson(){
    const type  = mainFieldType?.value || 'text';
    const label = mainFieldLabel?.value || 'Target';
  ...
  function syncMainTypeHidden(){
    ...
    mainTypeHidden.value = mainFieldType?.value || 'text';
  }
  ...
  window.__smmServiceApplyRemoteFields__ = function(scope, additionalFields){
    try{
      ...
      additionalFields.forEach((f, idx) => {
        ...
        addField(scope, {
          active: 1,
          name: String(f.name || ('Field ' + (idx + 1))).trim(),
          type: mapRemoteType(f.type || 'text'),
          input: String(f.input || ('field_' + (idx + 1))).trim(),
          description: String(f.description || '').trim(),
          minimum: Number(f.minimum || 0),
          maximum: Number(f.maximum || 0),
          validation: String(f.validation || '').trim(),
          required: Number(f.required || 0),
          options: Array.isArray(f.options) ? f.options.join(',') : String(f.options || '').trim(),
        });
      });

      serializeFieldsInScope(scope);
    }catch(e){
      console.warn('smm applyRemoteFields failed', e);
    }
  };

  window.__smmServiceSetMainField__ = function(scope, type, label){
    try{
      if (!scope) return;
      ...
      console.warn('smm setMainField failed', e);
    }
  };
