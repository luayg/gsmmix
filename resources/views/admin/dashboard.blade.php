...
@section('title',$t('admin.dashboard.title','Live Operations Dashboard'))
@section('bodyClass','has-command-dashboard')
@section('content')
<div class="command-dashboard" data-refresh-seconds="60">
  <header class="command-head">
    <div><span class="command-eyebrow">{{ $t('admin.dashboard.kicker','REAL-TIME BUSINESS INTELLIGENCE') }}</span><h1>{{ $t('admin.dashboard.welcome','Welcome back, :name',['name'=>str(auth()->user()->name ?? 'Admin')->before(' ')]) }}</h1><p>{{ $t('admin.dashboard.subtitle','Live insights from your platform') }}</p></div>
    <div class="command-head-meta"><span class="live-refresh"><i></i> LIVE · <b id="refreshCountdown">60s</b><small>{{ $t('admin.dashboard.auto_refresh','Auto-refreshing') }}</small></span><span class="command-quote">“Smarter Tools. Stronger Connections.”<small>— GSM MIX</small></span><time><i class="far fa-calendar"></i><span>{{ now()->format('D, M d, Y') }}<small>{{ now()->format('H:i') }} ({{ config('app.timezone') }})</small></span></time></div>
  </header>
  ...
  <section class="command-grid command-performance">
    <article class="command-card profit-panel">
      <div class="panel-head"><div><h2><i class="fas fa-chart-simple"></i> {{ $t('admin.dashboard.monthly_profit','Monthly Profit') }}</h2><strong>${{ number_format($monthProfit,2) }}</strong><span class="trend {{ ($profitChange ?? 0)>=0?'up':'down' }}">{{ ($profitChange ?? 0)>=0?'↑':'↓' }} {{ $profitChange===null?'—':abs($profitChange).'%' }} <small>{{ $t('admin.dashboard.vs_last_month','vs last month') }}</small></span></div><span class="year-chip">{{ now()->year }} <i class="fas fa-chevron-down"></i></span></div>
    ...
    @foreach([[$t('admin.dashboard.acceptance_rate','Order Acceptance Rate'),$acceptanceRate,'accepted',$statuses['success'],'green'],[$t('admin.dashboard.rejection_rate','Order Rejection Rate'),$rejectionRate,'rejected',$statuses['rejected']+$statuses['cancelled'],'red']] as [$label,$rate,$status,$count,$tone])
      <article class="command-card rate-card"><h2><i class="fas {{ $tone==='green'?'fa-user-check':'fa-circle-xmark' }}"></i> {{ $label }}</h2><div class="rate-body"><div class="ring tone-{{ $tone }}" style="--rate:{{ min(100,$rate) }}"><span>{{ $rate }}%</span></div><div class="rate-legend"><span><i class="dot {{ $tone }}"></i>{{ ucfirst($status) }}<strong>{{ number_format($count) }}</strong></span><span><i class="dot muted"></i>{{ $t('admin.dashboard.total','Total') }}<strong>{{ number_format(array_sum($statuses)) }}</strong></span></div></div></article>
    @endforeach
  </section>

  <section class="command-grid command-live-row">
    <article class="command-card compact-panel"><div class="panel-title"><h2><i class="fas fa-clock text-warning"></i> {{ $t('admin.dashboard.pending_approval','Pending Approval') }} <b>{{ $pending->count() }}</b></h2><a href="{{ route('admin.orders.imei.index') }}">{{ $t('common.view_all','View all') }}</a></div><div class="command-list">@forelse($pending as $order)<div><strong>#ORD-{{ $order->id }}</strong><span>{{ strtoupper($order->type) }} · {{ $order->customer }}</span><time>{{ \Illuminate\Support\Carbon::parse($order->created_at)->diffForHumans(short:true) }}</time><a class="mini-action" href="{{ $order->admin_url }}">Review</a></div>@empty<p class="empty-state">{{ $t('admin.dashboard.no_pending','No orders are waiting for approval.') }}</p>@endforelse</div></article>
    <article class="command-card compact-panel"><div class="panel-title"><h2><i class="fas fa-triangle-exclamation text-danger"></i> {{ $t('admin.dashboard.payment_alerts','Payment Review Alerts') }} <b>{{ $reviewPayments }}</b></h2><a href="{{ route('admin.finances.payment-reviews.index') }}">{{ $t('common.view_all','View all') }}</a></div><div class="command-list">@forelse($paymentAlerts as $payment)<div><strong>#PAY-{{ $payment->id }}</strong><span>${{ number_format($payment->amount_base,2) }} · User #{{ $payment->user_id }}</span><time>{{ \Illuminate\Support\Carbon::parse($payment->created_at)->diffForHumans(short:true) }}</time><a class="mini-action" href="{{ route('admin.finances.payment-reviews.index') }}">Review</a></div>@empty<p class="empty-state">{{ $t('admin.dashboard.no_payment_alerts','No payments require review.') }}</p>@endforelse</div></article>
    <article class="command-card online-panel"><div class="panel-title"><h2><i class="fas fa-users"></i> {{ $t('admin.dashboard.online_users','Online Users') }}</h2><span class="online-now"><i></i> Live</span></div><div class="connection-map" aria-hidden="true"><svg viewBox="0 0 420 150"><path d="M25 52l31-22 53 8 19 22-24 15-31-8-22 14-18-10zm116 9 22-19 37 3 15 15-12 15 18 18-14 42-21-10-7-33-31-12zm91-23 40-21 76 14 22 21-27 11-17-9-18 11-25-4-15 18-25-8zm75 57 38 8 24 25-21 11-32-15z"/></svg><span class="map-pulse p1"></span><span class="map-pulse p2"></span><span class="map-pulse p3"></span></div><div class="session-list">@forelse($online->take(4) as $session)<span><i></i>User #{{ $session->user_id }}<code>{{ $session->ip_address }}</code></span>@empty<span class="empty-state">No active database sessions.</span>@endforelse</div></article>
  </section>

  <section class="command-grid command-bottom">
    <article class="command-card recent-panel"><div class="panel-title"><h2><i class="fas fa-receipt"></i> {{ $t('admin.dashboard.recent_orders','Recent Orders') }}</h2><span>{{ number_format($monthOrders) }} {{ $t('admin.dashboard.this_month','this month') }}</span></div><div class="table-responsive"><table class="command-table"><thead><tr><th>Order</th><th>Customer</th><th>Service</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>@forelse($recentOrders as $order)<tr><td><b>#ORD-{{ $order->id }}</b></td><td>{{ $order->customer }}</td><td>{{ strtoupper($order->type) }}</td><td>${{ number_format($order->amount,2) }}</td><td><span class="status-pill is-{{ $order->status }}">{{ ucfirst($order->status) }}</span></td><td>{{ \Illuminate\Support\Carbon::parse($order->created_at)->format('M d, H:i') }}</td></tr>@empty<tr><td colspan="6" class="empty-state">No recent orders.</td></tr>@endforelse</tbody></table></div></article>
    <article class="command-card activity-panel"><div class="panel-title"><h2><i class="fas fa-wave-square"></i> {{ $t('admin.dashboard.live_activity','Live Activity') }}</h2><a href="{{ route('admin.logs.activity') }}">{{ $t('common.view_all','View all') }}</a></div><div class="activity-line">@foreach($recentOrders->take(3) as $order)<div><time>{{ \Illuminate\Support\Carbon::parse($order->created_at)->format('H:i') }}</time><i class="fas fa-cart-shopping"></i><span>Order <b>#ORD-{{ $order->id }}</b> {{ $order->status }}</span></div>@endforeach @foreach($recentUsers->take(3) as $user)<div><time>{{ $user->created_at->format('H:i') }}</time><i class="fas fa-user-plus"></i><span>New registration <b>{{ $user->name }}</b></span></div>@endforeach @if($recentOrders->isEmpty() && $recentUsers->isEmpty())<p class="empty-state">{{ $t('admin.dashboard.no_activity','No recent activity.') }}</p>@endif</div></article>
  </section>
</div>
@endsection
